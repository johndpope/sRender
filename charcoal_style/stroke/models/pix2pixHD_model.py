...
def img_patch(img):
    n, c, w, h = img.shape  # [1,1,250,200]
    x = math.ceil(w / 16)
    y = math.ceil(h / 16)
    img_patch = torch.zeros((x * y * n, c, 16, 16))  # [500,3,5,5]
    t = 0
    for k in range(n):
        for i in range(x):
            for j in range(y):
                img_patch[t] = img[k, :, 16 * i:16 * (i + 1),
                               16 * j:16 * (j + 1)]
                t = t + 1
    return img_patch
...
class Pix2PixHDModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.net_C.load_state_dict(torch.load("/home/meimei/mayme/data/stroke7_result/checkpoints/net_C_ins180.pth", map_location='cuda:1'))
    ...
    def encode_input(self, label_map, inst_map=None, real_image=None, feat_map=None, infer=False):
        ...
        input_label = Variable(input_label, volatile=infer)  ###`with torch.no_grad()
    ...
    def forward(self, label, inst, image, feat, infer=False):
        ...
        pred_fake_pool = self.discriminate(input_label[:, 0, :, :].unsqueeze(0), fake_image, use_pool=True)
        loss_D_fake = self.criterionGAN(pred_fake_pool, False)

        # Real Detection and Loss
        pred_real = self.discriminate(input_label[:, 0, :, :].unsqueeze(0), real_image)
        loss_D_real = self.criterionGAN(pred_real, True)

        # GAN loss (Fake Passability Loss)
        pred_fake = self.netD.forward(torch.cat((input_label[:, 0, :, :].unsqueeze(0), fake_image), dim=1))
        loss_G_GAN = self.criterionGAN(pred_fake, True)

        fakeB_patch = img_patch(fake_image)
        fakeB_patch = fakeB_patch.type(torch.FloatTensor).cuda()
        fake_label = self.net_C(fakeB_patch)
        realB_patch = img_patch(real_image)
        realB_patch = realB_patch.type(torch.FloatTensor).cuda()
        real_label = self.net_C(realB_patch)
        loss_C = 0
        for f in range(1, 5):
            loss_C += self.criterionMse(fake_label[f], real_label[f])
        loss_C *= 2e-3 ###storke loss
        ...
        fake_image = fake_image.expand(b, 3, h, w)
        real_image = real_image.expand(b, 3, h, w)
